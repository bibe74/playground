...
DECLARE @sysindexphysicalstats TABLE (
    ...
    index_id                int NOT NULL,
    partition_number        int NOT NULL,
    alloc_unit_type_desc    nvarchar(60) COLLATE database_default NOT NULL,
...
DECLARE @spt_values_O9T TABLE (
	[name]		nvarchar(35) COLLATE database_default NOT NULL,
...
IF (@is_azure_sql_db=1)
	...
	EXEC('SELECT DISTINCT [type]+N'': ''+LOWER([type_desc]) FROM sys.objects');
...
WHERE st.objectid=@object_id AND st.[dbid]=DB_ID(@database)
    AND qs._rownum=1;
...
IF (@has_cols_or_params=1) BEGIN;
	SELECT (CASE WHEN col.parameter_id IS NOT NULL AND
	       ...
	       (CASE WHEN col.[name] LIKE '%[^0-9a-z\_@]%' ESCAPE '\' OR col.[name] IN (SELECT [name] COLLATE database_default FROM sys.types) THEN QUOTENAME(col.[name]) ELSE col.[name] END)+
...
IF (@has_indexes=1)
	WITH ixc AS (
		SELECT ic.index_id, ic.is_included_column,
		       ...
		       (CASE WHEN c.[name] LIKE '%[^0-9a-z\_@]%' ESCAPE '\' OR c.[name] IN (SELECT [name] COLLATE database_default FROM sys.types) THEN QUOTENAME(c.[name]) ELSE c.[name] END)+(CASE WHEN ic.is_descending_key=1 THEN ' DESC' ELSE '' END) AS name
...
IF (@has_foreign_keys=1)
	WITH cols AS (
		SELECT fkc.constraint_object_id, fkc.constraint_column_id,
		       CAST((CASE WHEN pc.[name] LIKE '%[^0-9a-z\_@]%' ESCAPE '\' OR pc.[name] IN (SELECT [name] COLLATE database_default FROM sys.types) THEN QUOTENAME(pc.[name]) ELSE pc.[name] END) AS varchar(max)) AS parent_cols,
		       CAST((CASE WHEN rc.[name] LIKE '%[^0-9a-z\_@]%' ESCAPE '\' OR rc.[name] IN (SELECT [name] COLLATE database_default FROM sys.types) THEN QUOTENAME(rc.[name]) ELSE rc.[name] END) AS varchar(max)) AS referenced_cols
